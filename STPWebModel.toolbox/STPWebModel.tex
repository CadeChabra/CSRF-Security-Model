...
\@x{}\moduleLeftDash\@xx{ {\MODULE} STPWebModel}\moduleRightDash\@xx{}%
\@pvspace{8.0pt}%
\@x{ {\EXTENDS} Integers ,\, TLC ,\, Sequences}%
\@pvspace{8.0pt}%
\@x{ {\CONSTANT} HonestKey ,\, AtkKey ,\, HonestSessionId}%
\@pvspace{8.0pt}%
\pcalsymbolstrue
\pcalshadingtrue
\csyntaxfalse
\@x{\@s{12.29} {\p@mmalgorithm} STPWebModel}%
\@x{ {\p@variables}}%
\@x{\@s{16.4} MessageLog \.{=} \{ \} {\p@semicolon}}%
 \@x{\@s{16.4} HonestBrowser \.{=} [ source \.{\mapsto}\@w{honest} ,\, Cookies
 \.{\mapsto} \{ \} ,\, Headers \.{\mapsto} \{ \} ] {\p@semicolon}}%
 \@x{\@s{16.4} HonestServerState \.{=} [ SessionIds \.{\mapsto} \{ \} ,\,
 Tokens \.{\mapsto} \{ \} ] {\p@semicolon}}%
 \@x{\@s{16.4} AttackerServerState \.{=} [ SessionIds \.{\mapsto} \{ \} ,\,
 Tokens \.{\mapsto} \{ \} ] {\p@semicolon}}%
 \@x{\@s{16.4} HonestSecretKey \.{=} [ secret \.{\mapsto} \{ HonestKey \} ]
 {\p@semicolon}}%
 \@x{\@s{16.4} AttackerSecretKey \.{=} [ secret \.{\mapsto} \{ AtkKey \} ]
 {\p@semicolon}}%
 \@x{\@s{16.4} AttackerState \.{=} [ SessionIds\@s{0.51} \.{\mapsto} \{ \} ]
 {\p@semicolon}}%
 \@x{\@s{16.4} AttackerBrowser \.{=} [ source \.{\mapsto}\@w{attacker} ,\,
 Cookies \.{\mapsto} \{ \} ,\, Headers \.{\mapsto} \{ \} ]}%
\@pvspace{8.0pt}%
\@x{ {\p@define}}%
\@pvspace{8.0pt}%
\@x{\@s{16.4} HonestAccepted \.{\defeq}}%
\@x{\@s{32.8} \A\, msg \.{\in} MessageLog \.{:}}%
 \@x{\@s{44.12} msg . request . source \.{=}\@w{honest} \.{\implies} msg .
 response . status \.{=}\@w{success}}%
\@x{\@s{16.4} MaliciousRejected \.{\defeq}}%
\@x{\@s{32.8} \A\, msg \.{\in} MessageLog \.{:}}%
 \@x{\@s{44.12} msg . request . source \.{=}\@w{attacker} \.{\implies} msg .
 response . status \.{=}\@w{error}}%
\@pvspace{16.0pt}%
 \@x{\@s{16.4} HasValidSessionId ( req ,\, srv ) \.{\defeq} \E\, SessionId
 \.{\in} srv . SessionIds \.{:} \E\, cookie \.{\in} req . cookies \.{:}
 SessionId \.{=} cookie}%
\@x{}%
\@y{\@s{7.5}%
 \ensuremath{HasValidToken(req,\, srv) \.{\defeq} \E\,} Token
 \ensuremath{\.{\in} srv.Tokens}: \ensuremath{\E\,} Header
 \ensuremath{\.{\in} req.headers}: Token \ensuremath{\.{=}} Header
}%
\@xx{}%
 \@x{\@s{16.4} HasValidToken ( req ,\, srv ) \.{\defeq} ( srv . Tokens \.{=}
 \{ HonestKey \} ) \.{=} ( req . cookies \.{=} \{ HonestSessionId \} )}%
 \@x{\@s{16.4} ServerRequest ( src ) \.{\defeq} [ source \.{\mapsto} src .
 source ,\, cookies \.{\mapsto} src . Cookies ,\, headers \.{\mapsto} src .
 Headers ]}%
 \@x{\@s{16.4} Response ( req ,\, srv )\@s{0.99} \.{\defeq} {\IF} (
 HasValidToken ( req ,\, srv ) ) \.{=} ( HasValidSessionId ( req ,\, srv ) )
 \.{\THEN} [ destination \.{\mapsto} req . source ,\, status
 \.{\mapsto}\@w{success} ] \.{\ELSE} [ destination \.{\mapsto} req . source
 ,\, status \.{\mapsto}\@w{error} ]}%
\@pvspace{8.0pt}%
\@x{ {\p@end} {\p@define} {\p@semicolon}}%
\@pvspace{16.0pt}%
\@x{ {\p@procedure} LogMessagePair ( req ,\, resp ) {\p@begin}}%
\@x{\@s{16.4} Log\@s{.5}\textrm{:}\@s{3}}%
 \@x{\@s{16.4} MessageLog \.{:=} MessageLog \.{\cup} \{ [ request \.{\mapsto}
 req ,\, response \.{\mapsto} resp ] \} {\p@semicolon}}%
\@x{\@s{16.4} {\p@print} {\langle} MessageLog {\rangle} {\p@semicolon}}%
\@x{\@s{16.4} {\p@return} {\p@semicolon}}%
\@x{ {\p@end} {\p@procedure} {\p@semicolon}}%
\@pvspace{8.0pt}%
\@x{ {\p@process} SiteRequest \.{=} 1}%
\@x{ {\p@begin}}%
\@x{\@s{16.4} SSR\@s{.5}\textrm{:}\@s{3}}%
 \@x{\@s{16.4} {\p@print} {\langle}\@w{Same\ Site\ Req} {\rangle}
 {\p@semicolon}}%
\@pvspace{8.0pt}%
\@x{\@s{16.4} Session\@s{.5}\textrm{:}\@s{3}}%
 \@x{\@s{16.4} HonestBrowser . Cookies \.{:=} \{ HonestSessionId \}
 {\p@semicolon}}%
 \@x{\@s{16.4} HonestServerState . SessionIds \.{:=} HonestBrowser . Cookies
 {\p@semicolon}}%
\@pvspace{8.0pt}%
\@x{\@s{16.4} Tokens\@s{.5}\textrm{:}\@s{3}}%
 \@x{\@s{16.4} HonestServerState . Tokens \.{:=} HonestSecretKey . secret
 {\p@semicolon}}%
 \@x{\@s{16.4} HonestBrowser . Headers \.{:=} HonestServerState . Tokens
 {\p@semicolon}}%
 \@x{\@s{16.4} {\p@call} LogMessagePair ( ServerRequest ( HonestBrowser ) ,\,
 Response ( ServerRequest ( HonestBrowser ) ,\, HonestServerState ) )
 {\p@semicolon}}%
\@pvspace{8.0pt}%
\@x{ {\p@end} {\p@process} {\p@semicolon}}%
\@pvspace{8.0pt}%
\@x{ {\p@process} CrossSiteRequest \.{=} 2}%
\@x{ {\p@begin}}%
\@x{\@s{16.4} CSR\@s{.5}\textrm{:}\@s{3}}%
 \@x{\@s{16.4} {\p@print} {\langle}\@w{Cross\ Site\ Req} {\rangle}
 {\p@semicolon}}%
\@pvspace{8.0pt}%
\@x{\@s{16.4} HonestToAttacker\@s{.5}\textrm{:}\@s{3}}%
 \@x{\@s{16.4} HonestBrowser . Cookies \.{:=} \{ HonestSessionId \}
 {\p@semicolon}}%
 \@x{\@s{16.4} AttackerState . SessionIds \.{:=} HonestBrowser . Cookies
 {\p@semicolon}}%
 \@x{\@s{16.4} AttackerBrowser . Cookies \.{:=} AttackerState . SessionIds
 {\p@semicolon}}%
\@pvspace{8.0pt}%
\@x{\@s{16.4} Session\@s{.5}\textrm{:}\@s{3}}%
 \@x{\@s{16.4} AttackerServerState . SessionIds \.{:=} AttackerBrowser .
 Cookies {\p@semicolon}}%
\@pvspace{8.0pt}%
\@x{\@s{16.4} Tokens\@s{.5}\textrm{:}\@s{3}}%
 \@x{\@s{16.4} AttackerServerState . Tokens \.{:=} AttackerSecretKey . secret
 {\p@semicolon}}%
 \@x{\@s{16.4} AttackerBrowser . Headers \.{:=} AttackerServerState . Tokens
 {\p@semicolon}}%
 \@x{\@s{16.4} {\p@call} LogMessagePair ( ServerRequest ( AttackerBrowser )
 ,\, Response ( ServerRequest ( AttackerBrowser ) ,\, HonestServerState ) )
 {\p@semicolon}}%
\@pvspace{8.0pt}%
\@x{ {\p@end} {\p@process} {\p@semicolon}}%
\@pvspace{8.0pt}%
...
\@x{}%
\@y{\@s{0}%
 BEGIN TRANSLATION (\ensuremath{chksum(pcal) \.{=}} ``\ensuremath{1544d951}''
 \ensuremath{\.{\land} chksum(tla) \.{=}} ``\ensuremath{fc531894}'')
}%
\@xx{}%
\@x{}%
\@y{\@s{0}%
 Label \ensuremath{Session} of process \ensuremath{SiteRequest} at line 50
 col 5 changed to \ensuremath{Session\_
}}%
\@xx{}%
\@x{}%
\@y{\@s{0}%
 Label \ensuremath{Tokens} of process \ensuremath{SiteRequest} at line 54 col
 5 changed to \ensuremath{Tokens\_
}}%
\@xx{}%
\@x{ {\CONSTANT} defaultInitValue}%
 \@x{ {\VARIABLES} MessageLog ,\, HonestBrowser ,\, HonestServerState ,\,
 AttackerServerState ,\,}%
 \@x{\@s{51.42} HonestSecretKey ,\, AttackerSecretKey ,\, AttackerState ,\,
 AttackerBrowser ,\,}%
\@x{\@s{51.42} pc ,\, stack}%
\@pvspace{8.0pt}%
\@x{}%
...
\@xx{}%
\@x{ HonestAccepted \.{\defeq}}%
\@x{\@s{16.4} \A\, msg \.{\in} MessageLog \.{:}}%
 \@x{\@s{27.72} msg . request . source \.{=}\@w{honest} \.{\implies} msg .
 response . status \.{=}\@w{success}}%
\@x{ MaliciousRejected \.{\defeq}}%
\@x{\@s{16.4} \A\, msg \.{\in} MessageLog \.{:}}%
 \@x{\@s{27.72} msg . request . source \.{=}\@w{attacker} \.{\implies} msg .
 response . status \.{=}\@w{error}}%
\@pvspace{16.0pt}%
 \@x{ HasValidSessionId ( req ,\, srv ) \.{\defeq} \E\, SessionId \.{\in} srv
 . SessionIds \.{:} \E\, cookie \.{\in} req . cookies \.{:} SessionId \.{=}
 cookie}%
\@pvspace{8.0pt}%
 \@x{ HasValidToken ( req ,\, srv ) \.{\defeq} ( srv . Tokens \.{=} \{
 HonestKey \} ) \.{=} ( req . cookies \.{=} \{ HonestSessionId \} )}%
 \@x{ ServerRequest ( src ) \.{\defeq} [ source \.{\mapsto} src . source ,\,
 cookies \.{\mapsto} src . Cookies ,\, headers \.{\mapsto} src . Headers ]}%
 \@x{ Response ( req ,\, srv )\@s{0.99} \.{\defeq} {\IF} ( HasValidToken ( req
 ,\, srv ) ) \.{=} ( HasValidSessionId ( req ,\, srv ) ) \.{\THEN} [
 destination \.{\mapsto} req . source ,\, status \.{\mapsto}\@w{success} ]
 \.{\ELSE} [ destination \.{\mapsto} req . source ,\, status
 \.{\mapsto}\@w{error} ]}%
\@pvspace{8.0pt}%
\@x{ {\VARIABLES} req ,\, resp}%
\@pvspace{8.0pt}%
 \@x{ vars \.{\defeq} {\langle} MessageLog ,\, HonestBrowser ,\,
 HonestServerState ,\, AttackerServerState ,\,}%
 \@x{\@s{41.61} HonestSecretKey ,\, AttackerSecretKey ,\, AttackerState ,\,
 AttackerBrowser ,\,}%
\@x{\@s{41.61} pc ,\, stack ,\, req ,\, resp {\rangle}}%
\@pvspace{8.0pt}%
\@x{ ProcSet \.{\defeq} \{ 1 \} \.{\cup} \{ 2 \}}%
\@pvspace{8.0pt}%
\@x{ Init \.{\defeq}}%
\@y{\@s{0}%
 Global variables
...
\@xx{}%
\@x{\@s{35.70} \.{\land} MessageLog \.{=} \{ \}}%
 \@x{\@s{35.70} \.{\land} HonestBrowser \.{=} [ source \.{\mapsto}\@w{honest}
 ,\, Cookies \.{\mapsto} \{ \} ,\, Headers \.{\mapsto} \{ \} ]}%
 \@x{\@s{35.70} \.{\land} HonestServerState \.{=} [ SessionIds \.{\mapsto} \{
 \} ,\, Tokens \.{\mapsto} \{ \} ]}%
 \@x{\@s{35.70} \.{\land} AttackerServerState \.{=} [ SessionIds \.{\mapsto}
 \{ \} ,\, Tokens \.{\mapsto} \{ \} ]}%
 \@x{\@s{35.70} \.{\land} HonestSecretKey \.{=} [ secret \.{\mapsto} \{
 HonestKey \} ]}%
 \@x{\@s{35.70} \.{\land} AttackerSecretKey \.{=} [ secret \.{\mapsto} \{
 AtkKey \} ]}%
 \@x{\@s{35.70} \.{\land} AttackerState \.{=} [ SessionIds\@s{0.51}
 \.{\mapsto} \{ \} ]}%
 \@x{\@s{35.70} \.{\land} AttackerBrowser \.{=} [ source
 \.{\mapsto}\@w{attacker} ,\, Cookies \.{\mapsto} \{ \} ,\, Headers
 \.{\mapsto} \{ \} ]}%
\@x{\@s{35.70}}%
\@y{\@s{0}%
 Procedure \ensuremath{LogMessagePair
}}%
\@xx{}%
 \@x{\@s{35.70} \.{\land} req \.{=} [ self \.{\in} ProcSet \.{\mapsto}
 defaultInitValue ]}%
 \@x{\@s{35.70} \.{\land} resp \.{=} [ self\@s{4.66} \.{\in} ProcSet
 \.{\mapsto} defaultInitValue ]}%
 \@x{\@s{35.70} \.{\land} stack \.{=} [ self \.{\in} ProcSet \.{\mapsto}
 {\langle} {\rangle} ]}%
 \@x{\@s{35.70} \.{\land} pc \.{=} [ self \.{\in} ProcSet \.{\mapsto} {\CASE}
 self \.{=} 1 \.{\rightarrow}\@w{SSR}}%
\@x{\@s{160.39} {\Box}\@s{10.30} self \.{=} 2 \.{\rightarrow}\@w{CSR} ]}%
\@pvspace{8.0pt}%
\@x{ Log ( self ) \.{\defeq} \.{\land} pc [ self ] \.{=}\@w{Log}}%
 \@x{\@s{58.94} \.{\land} MessageLog \.{'} \.{=} ( MessageLog \.{\cup} \{ [
 request \.{\mapsto} req [ self ] ,\, response \.{\mapsto} resp [ self ] ] \}
 )}%
\@x{\@s{58.94} \.{\land} PrintT ( {\langle} MessageLog \.{'} {\rangle} )}%
 \@x{\@s{58.94} \.{\land} pc \.{'} \.{=} [ pc {\EXCEPT} {\bang} [ self ] \.{=}
 Head ( stack [ self ] ) . pc ]}%
 \@x{\@s{58.94} \.{\land} req \.{'} \.{=} [ req {\EXCEPT} {\bang} [ self ]
 \.{=} Head ( stack [ self ] ) . req ]}%
 \@x{\@s{58.94} \.{\land} resp \.{'} \.{=} [ resp {\EXCEPT} {\bang} [ self ]
 \.{=} Head ( stack [ self ] ) . resp ]}%
 \@x{\@s{58.94} \.{\land} stack \.{'} \.{=} [ stack {\EXCEPT} {\bang} [ self ]
 \.{=} Tail ( stack [ self ] ) ]}%
 \@x{\@s{58.94} \.{\land} {\UNCHANGED} {\langle} HonestBrowser ,\,
 HonestServerState ,\,}%
\@x{\@s{132.53} AttackerServerState ,\, HonestSecretKey ,\,}%
 \@x{\@s{132.53} AttackerSecretKey ,\, AttackerState ,\, AttackerBrowser
 {\rangle}}%
\@pvspace{8.0pt}%
\@x{ LogMessagePair ( self ) \.{\defeq} Log ( self )}%
\@pvspace{8.0pt}%
\@x{ SSR \.{\defeq} \.{\land} pc [ 1 ] \.{=}\@w{SSR}}%
\@x{\@s{37.81} \.{\land} PrintT ( {\langle}\@w{Same\ Site\ Req} {\rangle} )}%
 \@x{\@s{37.81} \.{\land} pc \.{'} \.{=} [ pc {\EXCEPT} {\bang} [ 1 ]
 \.{=}\@w{Session\_} ]}%
 \@x{\@s{37.81} \.{\land} {\UNCHANGED} {\langle} MessageLog ,\, HonestBrowser
 ,\, HonestServerState ,\,}%
 \@x{\@s{111.40} AttackerServerState ,\, HonestSecretKey ,\, AttackerSecretKey
 ,\,}%
 \@x{\@s{111.40} AttackerState ,\, AttackerBrowser ,\, stack ,\, req ,\, resp
 {\rangle}}%
\@pvspace{8.0pt}%
\@x{ Session\_ \.{\defeq} \.{\land} pc [ 1 ] \.{=}\@w{Session\_}}%
 \@x{\@s{56.35} \.{\land} HonestBrowser \.{'} \.{=} [ HonestBrowser {\EXCEPT}
 {\bang} . Cookies \.{=} \{ HonestSessionId \} ]}%
 \@x{\@s{56.35} \.{\land} HonestServerState \.{'} \.{=} [ HonestServerState
 {\EXCEPT} {\bang} . SessionIds \.{=} HonestBrowser \.{'} . Cookies ]}%
 \@x{\@s{56.35} \.{\land} pc \.{'} \.{=} [ pc {\EXCEPT} {\bang} [ 1 ]
 \.{=}\@w{Tokens\_} ]}%
 \@x{\@s{56.35} \.{\land} {\UNCHANGED} {\langle} MessageLog ,\,
 AttackerServerState ,\, HonestSecretKey ,\,}%
\@x{\@s{129.94} AttackerSecretKey ,\, AttackerState ,\, AttackerBrowser ,\,}%
\@x{\@s{129.94} stack ,\, req ,\, resp {\rangle}}%
\@pvspace{8.0pt}%
\@x{ Tokens\_ \.{\defeq} \.{\land} pc [ 1 ] \.{=}\@w{Tokens\_}}%
 \@x{\@s{54.62} \.{\land} HonestServerState \.{'} \.{=} [ HonestServerState
 {\EXCEPT} {\bang} . Tokens \.{=} HonestSecretKey . secret ]}%
 \@x{\@s{54.62} \.{\land} HonestBrowser \.{'} \.{=} [ HonestBrowser {\EXCEPT}
 {\bang} . Headers \.{=} HonestServerState \.{'} . Tokens ]}%
 \@x{\@s{54.62} \.{\land} \.{\land} req \.{'} \.{=} [ req {\EXCEPT} {\bang} [
 1 ] \.{=} ServerRequest ( HonestBrowser \.{'} ) ]}%
 \@x{\@s{65.73} \.{\land} resp \.{'} \.{=} [ resp {\EXCEPT} {\bang} [ 1 ]
 \.{=} Response ( ServerRequest ( HonestBrowser \.{'} ) ,\, HonestServerState
 \.{'} ) ]}%
 \@x{\@s{65.73} \.{\land} stack \.{'} \.{=} [ stack {\EXCEPT} {\bang} [ 1 ]
 \.{=} {\langle} [ procedure \.{\mapsto}\@s{4.1}\@w{LogMessagePair} ,\,}%
\@x{\@s{218.50} pc\@s{31.36} \.{\mapsto}\@s{4.10}\@w{Done} ,\,}%
\@x{\@s{218.50} req\@s{27.84} \.{\mapsto}\@s{4.1} req [ 1 ] ,\,}%
\@x{\@s{218.50} resp\@s{22.99} \.{\mapsto}\@s{4.1} resp [ 1 ] ] {\rangle}}%
\@x{\@s{215.93} \.{\circ} stack [ 1 ] ]}%
 \@x{\@s{54.62} \.{\land} pc \.{'} \.{=} [ pc {\EXCEPT} {\bang} [ 1 ]
 \.{=}\@w{Log} ]}%
 \@x{\@s{54.62} \.{\land} {\UNCHANGED} {\langle} MessageLog ,\,
 AttackerServerState ,\, HonestSecretKey ,\,}%
 \@x{\@s{128.20} AttackerSecretKey ,\, AttackerState ,\, AttackerBrowser
 {\rangle}}%
\@pvspace{8.0pt}%
\@x{ SiteRequest \.{\defeq} SSR \.{\lor} Session\_ \.{\lor} Tokens\_}%
\@pvspace{8.0pt}%
\@x{ CSR \.{\defeq} \.{\land} pc [ 2 ] \.{=}\@w{CSR}}%
\@x{\@s{39.34} \.{\land} PrintT ( {\langle}\@w{Cross\ Site\ Req} {\rangle} )}%
 \@x{\@s{39.34} \.{\land} pc \.{'} \.{=} [ pc {\EXCEPT} {\bang} [ 2 ]
 \.{=}\@w{HonestToAttacker} ]}%
 \@x{\@s{39.34} \.{\land} {\UNCHANGED} {\langle} MessageLog ,\, HonestBrowser
 ,\, HonestServerState ,\,}%
 \@x{\@s{112.93} AttackerServerState ,\, HonestSecretKey ,\, AttackerSecretKey
 ,\,}%
 \@x{\@s{112.93} AttackerState ,\, AttackerBrowser ,\, stack ,\, req ,\, resp
 {\rangle}}%
\@pvspace{8.0pt}%
 \@x{ HonestToAttacker \.{\defeq} \.{\land} pc [ 2 ]
 \.{=}\@w{HonestToAttacker}}%
 \@x{\@s{98.59} \.{\land} HonestBrowser \.{'} \.{=} [ HonestBrowser {\EXCEPT}
 {\bang} . Cookies \.{=} \{ HonestSessionId \} ]}%
 \@x{\@s{98.59} \.{\land} AttackerState \.{'}\@s{6.98} \.{=} [ AttackerState
 {\EXCEPT} {\bang} . SessionIds \.{=} HonestBrowser \.{'} . Cookies ]}%
 \@x{\@s{98.59} \.{\land} AttackerBrowser \.{'} \.{=} [ AttackerBrowser
 {\EXCEPT} {\bang} . Cookies \.{=} AttackerState \.{'} . SessionIds ]}%
 \@x{\@s{98.59} \.{\land} pc \.{'} \.{=} [ pc {\EXCEPT} {\bang} [ 2 ]
 \.{=}\@w{Session} ]}%
 \@x{\@s{98.59} \.{\land} {\UNCHANGED} {\langle} MessageLog ,\,
 HonestServerState ,\,}%
\@x{\@s{172.18} AttackerServerState ,\, HonestSecretKey ,\,}%
\@x{\@s{172.18} AttackerSecretKey ,\, stack ,\, req ,\, resp {\rangle}}%
\@pvspace{8.0pt}%
\@x{ Session \.{\defeq} \.{\land} pc [ 2 ] \.{=}\@w{Session}}%
 \@x{\@s{51.85} \.{\land} AttackerServerState \.{'} \.{=} [
 AttackerServerState {\EXCEPT} {\bang} . SessionIds \.{=} AttackerBrowser .
 Cookies ]}%
 \@x{\@s{51.85} \.{\land} pc \.{'} \.{=} [ pc {\EXCEPT} {\bang} [ 2 ]
 \.{=}\@w{Tokens} ]}%
 \@x{\@s{51.85} \.{\land} {\UNCHANGED} {\langle} MessageLog ,\, HonestBrowser
 ,\, HonestServerState ,\,}%
\@x{\@s{125.44} HonestSecretKey ,\, AttackerSecretKey ,\, AttackerState ,\,}%
\@x{\@s{125.44} AttackerBrowser ,\, stack ,\, req ,\, resp {\rangle}}%
\@pvspace{8.0pt}%
\@x{ Tokens \.{\defeq} \.{\land} pc [ 2 ] \.{=}\@w{Tokens}}%
 \@x{\@s{50.12} \.{\land} AttackerServerState \.{'} \.{=} [
 AttackerServerState {\EXCEPT} {\bang} . Tokens \.{=} AttackerSecretKey .
 secret ]}%
 \@x{\@s{50.12} \.{\land} AttackerBrowser \.{'} \.{=} [ AttackerBrowser
 {\EXCEPT} {\bang} . Headers \.{=} AttackerServerState \.{'} . Tokens ]}%
 \@x{\@s{50.12} \.{\land} \.{\land} req \.{'} \.{=} [ req {\EXCEPT} {\bang} [
 2 ] \.{=} ServerRequest ( AttackerBrowser \.{'} ) ]}%
 \@x{\@s{61.23} \.{\land} resp \.{'} \.{=} [ resp {\EXCEPT} {\bang} [ 2 ]
 \.{=} Response ( ServerRequest ( AttackerBrowser \.{'} ) ,\,
 HonestServerState ) ]}%
 \@x{\@s{61.23} \.{\land} stack \.{'} \.{=} [ stack {\EXCEPT} {\bang} [ 2 ]
 \.{=} {\langle} [ procedure \.{\mapsto}\@s{4.1}\@w{LogMessagePair} ,\,}%
\@x{\@s{214.00} pc\@s{31.36} \.{\mapsto}\@s{4.10}\@w{Done} ,\,}%
\@x{\@s{214.00} req\@s{27.84} \.{\mapsto}\@s{4.1} req [ 2 ] ,\,}%
\@x{\@s{214.00} resp\@s{22.99} \.{\mapsto}\@s{4.1} resp [ 2 ] ] {\rangle}}%
\@x{\@s{211.43} \.{\circ} stack [ 2 ] ]}%
 \@x{\@s{50.12} \.{\land} pc \.{'} \.{=} [ pc {\EXCEPT} {\bang} [ 2 ]
 \.{=}\@w{Log} ]}%
 \@x{\@s{50.12} \.{\land} {\UNCHANGED} {\langle} MessageLog ,\, HonestBrowser
 ,\, HonestServerState ,\,}%
 \@x{\@s{123.70} HonestSecretKey ,\, AttackerSecretKey ,\, AttackerState
 {\rangle}}%
\@pvspace{8.0pt}%
 \@x{ CrossSiteRequest \.{\defeq} CSR \.{\lor} HonestToAttacker \.{\lor}
 Session \.{\lor} Tokens}%
\@pvspace{8.0pt}%
\@x{}%
\@y{\@s{0}%
...
\@pvspace{8.0pt}%
\@x{ Next \.{\defeq} SiteRequest \.{\lor} CrossSiteRequest}%
 \@x{\@s{52.13} \.{\lor} ( \E\, self \.{\in} ProcSet \.{:} LogMessagePair (
 self ) )}%
...
\end{cpar}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 \ensuremath{\.{\,\backslash\,}}* Last modified Sun \ensuremath{Apr} 23
 12:13:36 \ensuremath{EDT} 2023 by \ensuremath{Cade} \ensuremath{Chabra
}%
\end{cpar}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 \ensuremath{\.{\,\backslash\,}}* Last modified \ensuremath{Fri}
 \ensuremath{Apr} 21 11:11:49 \ensuremath{EDT} 2023 by \ensuremath{andyking
}%
\end{cpar}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 \ensuremath{\.{\,\backslash\,}}* Created Sat \ensuremath{Mar} 25 15:05:39
 \ensuremath{EDT} 2023 by \ensuremath{andyking
}%
\end{cpar}%
\end{lcom}%
\end{document}
